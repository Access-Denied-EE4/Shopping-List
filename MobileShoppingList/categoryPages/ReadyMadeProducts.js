...
const DairyProducts = () => {
  ...
  const readyMadeItemsCollectionRef=collection(db, "ready_items");
  ...
  useEffect(()=>{
    ...
    const getreadyMadeItems=async()=>{
      ...
      const data=await getDocs(readyMadeItemsCollectionRef);

      //setReadyMadeItems state to be the array from collection
      //map from each doc   and set equal to obejct in readyMadeItems array
      //...doc.data will return the fields of the item
      //then also add the id
     setReadyMadeItems(data.docs.map((doc)=> ({...doc.data(), id: doc.id})));
    };
    //call async function
    getreadyMadeItems();
  },[]);
  ...
  useEffect(()=>{
    ...
    const getImgUrl=async()=>{
      ...
      const imageArray=[];
      for(let i=0; i<readyMadeItems.length; ++i)
      {
          //get url for the image of the relevant itme
          const imgUrl=await getDownloadURL(ref(storage,readyMadeItems[i].img_url));
          //create objecr
          imageArray.push({name: `${readyMadeItems[i].name}`, url: `${imgUrl}`});
      }
  ...
  return (
    ...
    <SafeAreaView style = {styles.flex}>
      <ScrollView>
        <View>
          <Text style = {styles.title}>Ready Made Products</Text>
                  {
