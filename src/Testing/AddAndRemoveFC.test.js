...
import App from '../App';
import { GetCartItems ,removeItemFromCart} from "../contexts/AuthContext";
import AppTesting from '../AppTester';


describe ("Adding and removing from cart" ,()=>{
   ...
   fireEvent.click(back, { button: 0});
    ...
    test('Remove from cart', async () => {
      ...
      expect(cartItems.includes("Chicken Breast")).toBe(true);
      removeItemFromCart(cartItems,0);
      expect(cartItems.includes("Chicken Breast")).toBe(false);

      expect(cartItems.includes("Butter")).toBe(true);
      removeItemFromCart(cartItems,0);
      expect(cartItems.includes("Butter")).toBe(false);

      expect(cartItems.includes("Sprite")).toBe(true);
      removeItemFromCart(cartItems,0);
      expect(cartItems.includes("Sprite")).toBe(false);

      expect(cartItems.includes("Crunchie")).toBe(true);
      removeItemFromCart(cartItems,0);
      expect(cartItems.includes("Crunchie")).toBe(false);

      expect(cartItems.includes("Toilet paper")).toBe(true);
      removeItemFromCart(cartItems,0);
      expect(cartItems.includes("Toilet paper")).toBe(false);

      expect(cartItems.includes("Apple")).toBe(true);
      removeItemFromCart(cartItems,0);
      expect(cartItems.includes("Apple")).toBe(false);
